...
namespace Stake
{
    ...
    {
        private static void
        processFlowline(string nameLayer, ObjectId idPolyGuideline, BlockTableRecord ms, string nameXRef, ref List<POI> varPOI, out ObjectId idAlign)
        {
            ...
            Stake_AddProfile.makeProfile(idAlign, varPOI, "FLOWLINE", "ByLayout", false); //VERTICAL CONTROL IS SET FROM POINTS
            ...
            {
                Debug.Print(varPOI[i].Station + " " + varPOI[i].Elevation + " " + varPOI[i].Desc0 + "     " + varPOI[i].DescX);
            }
            ...
            Stake_AddProfile.makeProfile(idAlign, varPOI, "FLOWLINE", "ByLayout", true);
