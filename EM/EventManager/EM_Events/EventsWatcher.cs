...
namespace EventManager
{
	...
	public class EventsWatcher : System.Windows.Forms.Form
	{
		...
		public static EW_ApplicationEvents			m_appWatcher;
		public static EW_DynLinkerEvents			m_dynLinkerWatcher;
		public static EW_LayoutManEvents			m_layoutManWatcher;
		public static EW_PlotEvents				m_plotWatcher;
		public static EW_EditorEvents				m_editorWatcher;
		public static EW_DocManEvents				m_docManWatcher;
		public static EW_DocumentEvents			m_docWatcher;
		public static EW_DatabaseEvents			m_dbWatcher;

		public static EW_DocManForUI				m_docManForUI;
	   ...
	   BaseObjs.writeDebug(ex.Message + " EventsWatcher.cs: line: 440");
   }
		}
	   ...
	   BaseObjs.writeDebug(ex.Message + " EventsWatcher.cs: line: 455");
   }
		}
		...
		private void appWatcherOnDuty()
		{
			if(m_doApp)
			{
				if( m_appWatcher == null)
					m_appWatcher = new EW_ApplicationEvents();
				else
					m_appWatcher.Do();
			}
			else
		...
		private void plotWatcherOnDuty()
		{
			if(m_doPlot)
			{
				if(m_plotWatcher == null)
					m_plotWatcher = new EW_PlotEvents();
				else
					m_plotWatcher.Do();
			}
			else
		...
		private void layoutWatcherOnDuty()
		{
			if(m_doLayout)
			{
				if(m_layoutManWatcher == null)
					m_layoutManWatcher = new EW_LayoutManEvents();
				else
		...
		private void linkerWatcherOnDuty()
		{
			if(m_doLinker)
			{
				if(m_dynLinkerWatcher == null)
					m_dynLinkerWatcher = new EW_DynLinkerEvents();
				else
		...
		private void docManWatcherOnDuty()
		{
			if(m_doDocMan)
			{
				if(m_docManWatcher == null)
					m_docManWatcher = new EW_DocManEvents();
				else
					m_docManWatcher.Do();
			}
			else
		...
		private void docWatcherOnDuty()
		{
			...
			{
				if(m_docWatcher == null)
					m_docWatcher = new EW_DocumentEvents();
				else
					m_docWatcher.Do();
			}
			else
		...
		private void dbWatcherOnDuty()
		{
			...
			{
				if(m_dbWatcher == null)
					m_dbWatcher = new EW_DatabaseEvents();
				else
					m_dbWatcher.Do();
			}
			else
		...
		private void editorWatcherOnDuty()
		{
			try
			{
				if(m_editorWatcher == null)
					m_editorWatcher = new EW_EditorEvents();
	   ...
	   BaseObjs.writeDebug(ex.Message + " EventsWatcher.cs: line: 677");
   }
		}
